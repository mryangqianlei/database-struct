...
// 描述:
// 作者: Yang
// 日期: 2019-09-2019/9/28 17:35
// 版权: 山东高招教育科技有限公司 @Since 2019
//
//============================================================
package main
...
func main() {
	...
	for rows.Next() {
		...
		contains := strings.Contains(table.Name, "_")
		var struct_str string
		if contains {
			split := strings.Split("_", table.Name)
			struct_str = fmt.Sprintf("type %s struct { \n", strFirstToUpper(split[0])+strFirstToUpper(split[1]))
		...
		for cloumns.Next() {
			...
			b := strings.Contains(column.Columname, "_")

			if b {
				fmt.Println("包含了下划线")
				split1 := strFirstToUpper(strings.Split(column.Columname, "_")[0])
				split2 := strFirstToUpper(strings.Split(column.Columname, "_")[1])
				struct_str += "    " + split1+split2
			}else{
